...
async function HandlePolicy(policyObj){
    ...
    const updatedUser = ({
      ...
      policyName:selectedPolicy[0].policyName,
      maxCost : selectedPolicy[0].maxcost,
  ...
  return (
    ...
    <form className=' w-25  mx-auto mt-5'  onSubmit={handleSubmit(HandlePolicy)} style={{minHeight:'80vh'}}>
            ...
            <div className='mb-4'>
                 <select  {...register('policyId',{required:true})} className = "form-control">
              <option value="">Policy name</option>
{
            
              policyData.map((item) => {
                return  <option value={item.policyId}>{item.policyName}</option>
              })
            }
           ...
           {errors.policyId?.type==='required' && <p className='fs-8' >please select from the list</p> }
</div>
